  ...
  <div class="posts-list">
    <el-card v-for="post in filteredPosts" :key="post.id" class="post-card" @click="goToPost(post)" >
      ...
      <div class="post-actions">
        ...
        <el-icon :size="15" @click.stop="deleteThread(post.id)">
          <Delete/>
        </el-icon>
      </div>
    </el-card>
...
import {Like, Message, Star} from '@icon-park/vue-next';
import {getdelete_thread, getcourse, getdiscussionlike, getdelete_like} from '@/api/discussion';
...
// 跳转到帖子详情页
const goToPost = post => {
  router.push(`/tea-end/course/discussion/post/${post.id}`);
};
