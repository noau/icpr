              ...
              <el-card>
                <el-form :model="gradingForm">
                  ...
                  <el-form-item label="评语" label-position="top">
                    <v-md-editor v-model="gradingForm.feedback" height="200px"></v-md-editor>
                  </el-form-item>
...
import { getPeerReviewList, addPeerReviews } from '@/api/assignments.js';
...
const gradingForm = ref({
  ...
  grade: 0,
  feedback: '',
  reviewedAt: new Date().toISOString().slice(0, 19).replace('T', ' '), // 格式为 'YYYY-MM-DD HH:MM:SS'
  attachmentId: 0, // 附件id 如果附件id为空不允许提交
  submissionId: 0, // 提交作业的id
  reviewerId: localStorage.getItem('userId') // 评阅者id
});
...
const init = async () => {
  try {
    ...
    const data = await getPeerReviewList({
    ...
    if (tableData.value.length > 0) {
      //设置初始化数据
      gradingForm.value.attachmentId = data[0].attachments[0] ? data[0].attachments[0] : 0
      gradingForm.value.submissionId = data[0].id ? data[0].id : 0
      
      if(gradingForm.value.attachmentId != 0) {
        loadFileUrl(gradingForm.value.attachmentId)
      }
    } 
    console.log(gradingForm.value);
...
const submitGrading = async () => {
  ...
  try {
    await addPeerReviews(gradingForm.value);
    console.log('提交评分成功:');
    alert('评分提交成功');
...
const previousStudent = () => {
  ...
  if (currentStudentIndex.value > 0) {
    ...
    if (currentStudentIndex.value < tableData.value.length) {
    currentStudentIndex.value--;
    console.log("上一个currentStudentIndex:", currentStudentIndex.value);
    // updateSubmissionId(); // 更新 submissionId
    if(tableData.value[currentStudentIndex.value].attachments.length > 0){
      gradingForm.value.attachmentId = tableData.value[currentStudentIndex.value].attachments[0] ? tableData.value[currentStudentIndex.value].attachments[0] : 0
      gradingForm.value.submissionId = tableData.value[currentStudentIndex.value].id ? tableData.value[currentStudentIndex.value].id : 0
      if(gradingForm.value.attachmentId != 0) {
        loadFileUrl(gradingForm.value.attachmentId)
      }
...
const nextStudent = () => {
  ...
  if (currentStudentIndex.value < tableData.value.length - 1) {
    currentStudentIndex.value++;
    console.log("currentStudentIndex:", currentStudentIndex.value);
    // updateSubmissionId(); // 更新 submissionId
    if(tableData.value[currentStudentIndex.value].attachments.length > 0){
      gradingForm.value.attachmentId = tableData.value[currentStudentIndex.value].attachments[0] ? tableData.value[currentStudentIndex.value].attachments[0] : 0
      gradingForm.value.submissionId = tableData.value[currentStudentIndex.value].id ? tableData.value[currentStudentIndex.value].id : 0
