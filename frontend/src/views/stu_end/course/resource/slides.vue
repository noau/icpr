...
import { getppt } from '@/api/course.js';
...
function getpptlist() {
  ...
  getppt(id, token)
    ...
    .catch(error => {
      ElMessage.error('获取课件列表失败，请检查网络或联系管理员');
      console.error(error);
    });
}
...
function getpptlist() {
  const id = localStorage.getItem('kcid');
  const token = localStorage.getItem('token');
  console.log('获取课件列表，课程ID:', id, 'Token:', token);

  if (!id || !token) {
    console.error('课程ID或Token缺失');
    return;
  }
